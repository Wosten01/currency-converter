...
import CurrencySelector, { Option } from "../currency-converter/CurrencySelector";
import { currencyNames } from "../../data/currency";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../../store";
import { useGetRatesQuery } from "../../api/currencyApi";
import { setCurrencyFrom } from "../../store/currencySlice";
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
        {filteredCurrencies.length > 0 ? (
          filteredCurrencies.map(([value, label]) => (
            <div
              key={value}
              className="border p-4 rounded-lg shadow-md text-center flex flex-col justify-center"
            >
              <h2 className="text-3xl font-bold">
                {!error && !isLoading ? rates?.conversion_rates[value] || 0 : 0}
              </h2>
